...
const Checkout = () => {
  ...
  function handleRemoveProduct(product: any): any {
    const updatedProducts = value.products.filter(
  ...
  async function handleOnCheckout(): Promise<any> {
    const stripePromise = loadStripe(
      "pk_test_51NoS4DFDZ1Q35EswDCj7eksWLm9MvEva1DtSCKNWrXHGVO9hpyartkCXFgfi77YhucXCsG7T3yCKapxc8TZQqSJQ00pE6EfZjv"
    );
    const stripe = await stripePromise;
    await fetch("http://localhost:3000/checkout/payment", {
      method: "POST",
      headers: {
      ...
      .then(async (data) => {
        ...
        // Lägga till produkterna vi valt i databasen
        fetch(
          "http://localhost:3000/order_success/" +
  ...
  return (
    <div className="sm:w-2/3 mx-auto bg-white mt-10 p-4">
      ...
      {value.products.map((product, index) => (
        <div
          key={index}
          className="flex gap-4 justify-between align-center mt-4 border border-b-slate-200 p-2"
        >
          <div className="w-28">
      ...
      <button
        ...
        className="bg-purple-600 text-white p-2 mt-4 block mx-auto w-24"
      >
        Beställ
      </button>
    </div>
